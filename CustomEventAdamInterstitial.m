                                   ...
                                   request:(GADCustomEventRequest *)customEventRequest
{
    ...
    _interstitialAd.clientId = serverParameter; // AdMob mediation UI상에 입력한 값이 serverParameter 인자로 전달됨. Adam의 경우, Adam 승인 이전에는 "InterstitialTestClientId"를 지정해야 테스트가 가능.
    [_interstitialAd setDelegate:self];
    [_interstitialAd requestAndPresent];    // 전면광고를 호출하고 노출함. AdMob custom event 구현상 보여주는 것은 전면광고 요청이 성공했을 때 진행하도록 되어 있으나 Adam SDK가 호출과 노출을 하나로 처리하고 있어서 예외적으로 구현함.
}
